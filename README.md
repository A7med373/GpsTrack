...
Веб-сервис для получения координат GPS-трекера, сохранения точек в SQLite и отображения текущего положения на плане здания.

![Карта геотрекинга](assets/geotracking-map.svg)

Проект сделан как MVP под конкретный трекер и конкретное здание. Ниже описано, как заменить трекер, карту и координаты здания.

## Возможности

- Получение координат из API 365gps.net.
- Сохранение истории координат в SQLite.
- Отображение точки на плане здания.
- Проверка попадания точки в границы здания или буферную зону.
- Настраиваемый интервал обновления координат.
